...
export default function DashboardPage({ status, showToast, onNavigate, onOpenCheckerWithUrl }) {
  ...
  const [strategyChanges, setStrategyChanges] = useState([]);
...
function AvailChart({ series, strategyChanges, now, intervalMin, t }) {
  const wrapRef = useRef(null);
  const [w, setW] = useState(0);
  const [hover, setHover] = useState(null); // { x, pts: [{label,color,pct,ts}], strategyChange: {...} }
  ...
  const data = useMemo(() => {
    ...
    // Маркеры смены стратегий (оранжевые вертикальные штрихи)
    const stratMarkers = [];
    if (strategyChanges && strategyChanges.length > 0) {
      for (const ch of strategyChanges) {
        const ts = ch.timestamp instanceof Date ? ch.timestamp.getTime() : new Date(ch.timestamp).getTime();
        if (!isFinite(ts) || ts < gridStart || ts > gridEnd) continue;
        stratMarkers.push({
          x: xOf(ts),
          ts,
    ...
    return { lines, ticks, nowX, gridStart, gridEnd, span, stratMarkers };
  }, [series, strategyChanges, now, plotW, intervalMin]);
  ...
  const onMove = (e) => {
    ...
    // Проверяем ближайший маркер стратегии
    let bestMarker = null, bestMarkerDist = Infinity;
    if (data.stratMarkers) {
      for (const m of data.stratMarkers) {
        const d = Math.abs(m.x - mx);
        if (d < bestMarkerDist) { bestMarkerDist = d; bestMarker = m; }
      }
    }
    const showMarker = bestMarker && bestMarkerDist < 20;
    if (bestTs === null && !showMarker) { setHover(null); return; }
    const hx = Math.max(padL, Math.min(padL + plotW, data.lines[0] ? (padL + ((bestTs - gridStart) / span) * plotW) : (showMarker ? bestMarker.x : mx)));
    const pts = bestTs ? data.lines
      .map((l) => {
      ...
      .filter(Boolean) : [];
    setHover({ x: hx, pts, strategyChange: showMarker ? bestMarker : null });
  };
  ...
  return (
    <div className="avail-chart" ref={wrapRef}>
      ...
      >
        ...
        {/* Маркеры смены стратегий (оранжевые вертикальные штрихи) */}
        {data.stratMarkers && data.stratMarkers.map((m, i) => (
          <g key={`sm${i}`}>
            <line x1={m.x} y1={padT} x2={m.x} y2={padT + plotH}
              stroke="var(--warning)" strokeWidth="1.5" strokeDasharray="4 3" opacity="0.8" />
            <circle cx={m.x} cy={padT} r="3" fill="var(--warning)" />
          </g>
        ))}
      ...
      {hover && (hover.pts.length > 0 || hover.strategyChange) && (
        <div className="avail-tooltip" style={{ left: Math.min(Math.max(hover.x, 60), (w || 300) - 60) }}>
          {hover.strategyChange && (
            <span className="avail-tooltip-strat">
              <span className="avail-tooltip-strat-dot" />
              {hover.strategyChange.strategy}
              {hover.strategyChange.source ? ` (${hover.strategyChange.source})` : ''}
            </span>
          )}
          {hover.pts.length > 0 && (
            <>
