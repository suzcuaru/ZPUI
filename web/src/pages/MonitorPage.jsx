...
export default function MonitorPage({ status }) {
  const { t } = useT();
  const [liveHistory, setLiveHistory] = useState({ dl: [], ul: [] });
  const [devices, setDevices] = useState([]);
  const statusTick = useRef(0);

  const m = status?.monitor || {};
  const proxyStats = status?.proxy?.stats || {};

  const loadData = useCallback(async () => {
    const devs = await api('GET', '/api/monitor/devices');
  ...
  useEffect(() => {
    ...
    setLiveHistory(prev => {
      ...
      if (newDl.length > MAX_SAMPLES) newDl.shift();
      if (newUl.length > MAX_SAMPLES) newUl.shift();
      return { dl: newDl, ul: newUl };
    });
  }, [status]);

  const proxyDl = proxyStats.total_bytes_recv || 0;
  const proxyUl = proxyStats.total_bytes_sent || 0;
  const proxyConns = proxyStats.active_connections || 0;
    ...
    <>
        ...
        <div className="mon-stat">
          ...
          <div className="mon-stat-split">
            <div className="mon-stat-split-item">
              ...
              <span className="mono">{formatBytes(proxyDl)}</span>
            </div>
            <div className="mon-stat-split-item">
              <span className="mon-stat-split-tag ul">UP</span>
              <span className="mono">{formatBytes(proxyUl)}</span>
            </div>
          </div>
        </div>
        <div className="mon-stat">
      ...
      <div className="section mon-devices-section">
        <div className="section-title">
          {t('monitor.devices')}
          {proxyConns > 0 && <span className="mon-device-conns-badge">{proxyConns} {t('monitor.activeConns')}</span>}
        </div>
        {devices.length > 0 ? (
          <div className="mon-devices">
            {devices.map(d => (
              <div key={(d.mac || '') + d.ip} className="mon-device">
