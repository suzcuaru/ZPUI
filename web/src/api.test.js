...
describe('api routing', () => {
  ...
  it('parses query params for GET /api/logs', async () => {
    await api('GET', '/api/logs?table=zapret&level=ERROR&limit=50&offset=0');
    expect(app.GetLogs).toHaveBeenCalledWith('zapret', 'ERROR', 50, 0);
  });

  it('applies defaults when query params are missing', async () => {
    await api('GET', '/api/logs');
    expect(app.GetLogs).toHaveBeenCalledWith('app', '', 200, 0);
  });
