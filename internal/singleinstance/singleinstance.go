...
func Check(exePath string) (cleanup func(), err error) {
	...
	if otherPID != 0 {
		title, _ := windows.UTF16PtrFromString("ZPUI — Already Running")
		msg, _ := windows.UTF16PtrFromString(
			fmt.Sprintf("ZPUI is already running (PID: %d).\n\nClose the other instance and open this one?", otherPID),
		)
		...
		if btn == idYes {
			...
			title2, _ := windows.UTF16PtrFromString("Error")
			msg2, _ := windows.UTF16PtrFromString("Failed to close the other ZPUI instance.\nPlease close it manually in Task Manager.")
