...
func getActiveAdapters() []string {
	psCmd := "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; (Get-NetAdapter | Where-Object Status -eq 'Up').Name"
	cmd := executil.HiddenCmd("powershell", "-NoProfile", "-Command", psCmd)
...
func getDefaultAdapter() []string {
	psCmd := "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; (Get-NetAdapter | Where-Object Status -eq 'Up' | Select-Object -First 1).Name"
	cmd := executil.HiddenCmd("powershell", "-NoProfile", "-Command", psCmd)
	output, err := cmd.Output()
	if err != nil {
