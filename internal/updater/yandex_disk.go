...
//     /selfupdate.exe          - update module
//     /report.exe              - diagnostics report module
...
func FetchRemoteVersionsYandex(publicKey string) (*RemoteVersions, error) {
        if publicKey == "" {
...
// YandexZapretVersion возвращает версию zapret с Яндекс.Диска по имени самой
// свежей папки «Zapret <version>» в корне публичной папки. Не зависит от
// генерации zip-URL (которая может недоступна), поэтому работает даже когда
// YandexFindZapretZip возвращает пустую ссылку.
func YandexZapretVersion(publicKey string) string {
	if publicKey == "" {
		return ""
	}
	items, err := listYandexDir(publicKey, "")
	...
	for _, it := range items {
		...
		if !strings.HasPrefix(lower, "zapret ") && lower != "zapret" {
			continue
		}
		v := extractVersionFromZapretFolder(it.Name)
		if v == "" {
			continue
		}
...
// ============================================================
// Independent component updates (/components/<name>/)
//
// Структура на Яндекс.Диске:
//   /components/<name>/version.txt  — версия модуля (одна строка)
//   /components/<name>/<name>.exe   — исполняемый файл
//   /components/<name>/checksum.sha256 — SHA-256 хеш файла
// ============================================================

// FetchComponentVersion получает версию модуля из /components/<name>/version.txt.
func FetchComponentVersion(name string) (string, error) {
	url, err := yandexDownloadURL(YandexPublicURL, "components/"+name, "version.txt")
	if err != nil {
		return "", fmt.Errorf("component %s version: %w", name, err)
	}
	if url == "" {
		return "", nil
	}
	tmpFile := os.TempDir() + "\\zpui_comp_ver.txt"
	if err := downloadFromYandex(url, tmpFile); err != nil {
		return "", err
	}
...
// DownloadComponentFile скачивает файл из /components/<name>/ на Яндекс.Диске.
func DownloadComponentFile(name, filename, dest string) error {
	url, err := yandexDownloadURL(YandexPublicURL, "components/"+name, filename)
	if err != nil {
		return fmt.Errorf("component %s file %s: %w", name, filename, err)
	}
	if url == "" {
		return fmt.Errorf("component %s: файл %s не найден на Яндекс.Диске", name, filename)
	}
	return downloadFromYandex(url, dest)
}

// ReplaceComponent заменяет один модуль (exe), скачав его из /components/<name>/.
// Старый файл переименовывается в .bak.
func ReplaceComponent(exeDir, name string) error {
	tmpDest := exeDir + "\\" + name + ".new"
	if err := DownloadComponentFile(name, name+".exe", tmpDest); err != nil {
		return err
	}
	oldPath := exeDir + "\\" + name + ".exe"
	bakPath := exeDir + "\\" + name + ".exe.bak"
	os.Remove(bakPath)
	if _, err := os.Stat(oldPath); err == nil {
		os.Rename(oldPath, bakPath)
	}
	return os.Rename(tmpDest, oldPath)
}
