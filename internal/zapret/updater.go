...
type VersionSources struct {
	...
	CloudVersion string `json:"cloud_version"`
	CloudAvail   bool   `json:"cloud_avail"`
}

func (m *Manager) FetchVersionSources() *VersionSources {
	res := &VersionSources{Current: m.version}

	if v := updater.YandexZapretVersion(updater.YandexPublicURL); v != "" {
		res.CloudVersion = v
		res.CloudAvail = true
	}
...
func (m *Manager) CheckForUpdates() (*UpdateInfo, error) {
	...
	if yaVersion != "" {
		if u, _, err := updater.YandexFindZapretZip(updater.YandexPublicURL); err == nil && u != "" {
			yaURL = u
		}
		m.log.Info("updater", "Found zapret on Yandex Disk: "+yaVersion)
	}

	info.Source = "yandex"
	info.LatestVersion = yaVersion
	...
	if info.LatestVersion == "" {
		return nil, fmt.Errorf("не удалось получить версию Запрета с Яндекс.Диска")
	}
...
func (m *Manager) PerformUpdate(progress chan<- UpdateProgress) error {
	...
	// Список URL уже упорядочен CheckForUpdates: основной источник (с более
	// свежей версией) первым, затем зеркала и альтернативный источник.
...
func (m *Manager) extractUpdate(zipPath string) error {
	...
	if len(failed) > 0 {
		m.log.Info("updater", fmt.Sprintf("%d файлов заняты, полная остановка процессов и повтор...", len(failed)))
		killWinws()
		...
		waitForProcessDeath("winws.exe", 8*time.Second)
		killWinws()
...
// waitForProcessDeath ждёт, пока указанный процесс исчезнет из tasklist.
func waitForProcessDeath(name string, timeout time.Duration) {
	deadline := time.Now().Add(timeout)
	for time.Now().Before(deadline) {
		cmd := executil.HiddenCmd("tasklist", "/FI", "IMAGENAME eq "+name, "/NH")
		output, _ := cmd.Output()
...
// thoroughCleanup полностью останавливает zapret и освобождает файлы:
// 1) остановка + удаление службы
// 2) убийство winws.exe
// 3) остановка + удаление драйверов WinDivert (выгрузка из ядра)
// 4) ожидание завершения процесса
// 5) повторная проверка
func (m *Manager) thoroughCleanup() {
	m.log.Info("updater", "thoroughCleanup: начало полной остановки")
	...
	waitForProcessDeath("winws.exe", 10*time.Second)

	killWinws()
	time.Sleep(3 * time.Second)

	m.log.Info("updater", "thoroughCleanup: остановка завершена")
}

// DownloadUpdate скачивает архив обновления zapret во временный файл с прогрессом.
// Возвращает путь к скачанному zip. Готовит список URL: GitHub + ghproxy + Yandex.
func (m *Manager) DownloadUpdate(progressFn ProgressFn) (string, error) {
	info, err := m.CheckForUpdates()
	if err != nil {
		return "", err
	}
	...
	return m.DownloadUpdateInfo(info, progressFn)
}

// DownloadUpdateInfo скачивает архив по уже готовому UpdateInfo (без повторной
// проверки). Позволяет вызывающему сначала узнать версию/источник через
// CheckForUpdates, показать их пользователю, и только потом качать.
func (m *Manager) DownloadUpdateInfo(info *UpdateInfo, progressFn ProgressFn) (string, error) {
	if info == nil || info.DownloadURL == "" {
		return "", fmt.Errorf("нет доступного URL для скачивания")
	}
	tempZip := filepath.Join(os.TempDir(), "zapret-update.zip")
	// URL уже упорядочены CheckForUpdates: свежий источник первым.
	urls := []string{info.DownloadURL}
	urls = append(urls, info.FallbackURLs...)
	...
	for _, u := range urls {
		...
		m.log.Info("updater", "Скачивание zapret: "+u)
		if err := downloadFileWithProgress(u, tempZip, progressFn); err != nil {
			m.log.Warn("updater", "Download failed ("+u+"): "+err.Error())
			lastErr = err
			continue
		}
		if err := verifyZip(tempZip); err != nil {
			m.log.Warn("updater", "Zip invalid ("+u+"): "+err.Error())
			lastErr = err
			os.Remove(tempZip)
			continue
		}
		m.log.Info("updater", "Zapret archive downloaded successfully")
		return tempZip, nil
	}
	if lastErr == nil {
		lastErr = fmt.Errorf("нет доступных URL для скачивания")
	}
	return "", lastErr
}

// InstallFromZip применяет обновление zapret из скачанного архива.
// Шаги: 1) бекап списков + захват стратегии службы, 2) остановка и удаление
// службы, 3) распаковка обновления, 4) восстановление списков, 5) установка
// службы с той же стратегией, что была до обновления. progress сообщает шаги.
func (m *Manager) InstallFromZip(zipPath string, progress func(step string, pct int)) error {
	...
	emit("Резервное копирование списков пользователя...", 10)
	snap := m.CaptureState()

	emit("Остановка и удаление службы...", 25)
	m.thoroughCleanup()

	emit("Установка обновления...", 45)
	if err := m.extractUpdate(zipPath); err != nil {
		emit("Восстановление предыдущего состояния...", 70)
		m.RestoreState(snap)
		return err
	}
	m.version = detectZapretVersion(m.cfg)

	emit("Восстановление списков и установка службы...", 75)
	if err := m.RestoreState(snap); err != nil {
		m.log.Warn("updater", "RestoreState error: "+err.Error())
	}

	emit("Завершено", 100)
	m.log.Info("updater", fmt.Sprintf("Zapret updated via zip, version: %s", m.version))
	return nil
}
