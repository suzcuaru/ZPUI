...
func (m *Manager) SetStrategy(filename string) error {
	...
	m.cfg.SetCurrentStrategy(filename)
...
func (m *Manager) LogStrategyChange(filename string, source string) {
	database.InsertActionLog(&database.ActionLog{
		Category: "zapret",
		Action:   "strategy_applied",
		Details:  filename + "|" + source,
	})
}
...
func (m *Manager) AutoSelectAndApply(ctx context.Context, results chan<- AutoTestResult, done chan<- struct{}) {
	...
	log := func(msg string) { m.log.Info("autoselect", msg) }
	logWarn := func(msg string) { m.log.Warn("autoselect", msg) }
	logErr := func(msg string) { m.log.Error("autoselect", msg) }
	...
	if autoTestActive {
		autoTestMu.Unlock()
		logErr("already running, aborting")
	...
	log(fmt.Sprintf("strategies: %d, resources: %d, original: %q", len(strategies), len(resources), originalStrategy))

	if len(strategies) == 0 {
		logErr("no strategies found")
	...
	for i, s := range strategies {
		select {
		case <-testCtx.Done():
			logWarn(fmt.Sprintf("[%d/%d] cancelled", i+1, len(strategies)))
			results <- AutoTestResult{Type: "info", Message: "Подбор отменён"}
			goto applyBest
		default:
		}

		log(fmt.Sprintf("[%d/%d] %s", i+1, len(strategies), s.Name))
		...
		strategyCtx, strategyCancel := context.WithTimeout(testCtx, 45*time.Second)

		strategyDone := make(chan error, 1)
		go func(fn string) {
			strategyDone <- m.SetStrategy(fn)
		}(s.Filename)

		var setErr error
		select {
		case setErr = <-strategyDone:
		case <-strategyCtx.Done():
			setErr = fmt.Errorf("таймаут применения стратегии")
		}

		if setErr != nil {
			strategyCancel()
			logWarn(fmt.Sprintf("[%d/%d] SetStrategy failed: %v", i+1, len(strategies), setErr))
			results <- AutoTestResult{Type: "result", Strategy: s.Filename, Error: setErr.Error()}
			continue
		}

		if !sleepCtx(strategyCtx, 4*time.Second) {
			strategyCancel()
			goto applyBest
		}
		...
		if status != StatusRunning {
			strategyCancel()
			logWarn(fmt.Sprintf("[%d/%d] service not running (status=%v)", i+1, len(strategies), status))
			results <- AutoTestResult{Type: "result", Strategy: s.Filename, Error: "служба/процесс не запустились"}
			continue
		}

		if !m.verifyStrategyApplied(s.Filename) {
			strategyCancel()
			logWarn(fmt.Sprintf("[%d/%d] strategy not verified", i+1, len(strategies)))
		...
		checkDone := make(chan *blockcheck.BulkReport, 1)
		go func() {
			checkDone <- checker.BulkCheck(resources, nil)
		}()

		var report *blockcheck.BulkReport
		select {
		case report = <-checkDone:
		case <-strategyCtx.Done():
			strategyCancel()
			logWarn(fmt.Sprintf("[%d/%d] проверка ресурсов превысила таймаут", i+1, len(strategies)))
			results <- AutoTestResult{Type: "result", Strategy: s.Filename, Error: "таймаут проверки ресурсов"}
			continue
		}
		...
		log(fmt.Sprintf("[%d/%d] result: %d/%d ok, %dms avg", i+1, len(strategies), okCount, len(resources), avgMs))
...
applyBest:
	log(fmt.Sprintf("applyBest: %d results", len(allResults)))
	if len(allResults) == 0 {
		logErr("no working strategies found")
	...
	best := allResults[0]
	log(fmt.Sprintf("best: %s (%d/%d, %dms)", best.Strategy, best.Ok, best.Total, best.AvgMs))
	...
	if err := m.SetStrategy(best.Strategy); err != nil {
		logErr(fmt.Sprintf("failed to apply best: %v", err))
	...
	status := m.GetStatus()
	if status != StatusRunning {
		logErr(fmt.Sprintf("best strategy crashed (status=%v)", status))
	...
	log(fmt.Sprintf("COMPLETE: applied %s", best.Strategy))

	resultsJSON, _ := json.Marshal(allResults)
	cur, _ := database.GetCurrentOperator()
	if cur != nil && cur.OperatorKey != "" {
		database.SaveOperatorStrategy(cur.OperatorKey, best.Strategy, string(resultsJSON))
		database.SetCurrentOperator(cur.OperatorKey, cur.OperatorName, best.Strategy)
		log(fmt.Sprintf("saved strategy %s for operator %s", best.Strategy, cur.OperatorName))
	}
