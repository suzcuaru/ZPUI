...
// StopRequested возвращает true, если последний останов инициирован вызовом Stop().
// Используется монитором здоровья, чтобы отличить ручной стоп от аварийного падения службы.
...
func (m *Manager) StartWithStrategy(strategyFile string) error {
	...
	if err := m.verifyEssential(); err != nil {
		return fmt.Errorf("zapret files incomplete: %w", err)
	}
	...
	// Close old stopCh to signal any previous streamOutput goroutines to exit
	if m.stopCh != nil {
		close(m.stopCh)
	}
...
type BackupSnapshot struct {
	...
	Strategy        string            `json:"strategy"`
	ServiceStrategy string            `json:"service_strategy"`
	WasService      bool              `json:"was_service"`
	Version         string            `json:"version"`
...
// ServiceStrategy фиксирует стратегию, с которой служба была УСТАНОВЛЕНА
// (из реестра), чтобы после обновления поставить службу с той же стратегией.
func (m *Manager) CaptureState() *BackupSnapshot {
	svcStatus := m.GetServiceStatus()
	snap := &BackupSnapshot{
		Timestamp:  time.Now(),
		Strategy:   m.cfg.GetCurrentStrategy(),
		Version:    m.GetVersion(),
		WasService: svcStatus.Installed,
		UserLists:  make(map[string]string),
	}

	if svcStrat := m.GetInstalledServiceStrategy(); svcStrat != "" {
		if !strings.HasSuffix(svcStrat, ".bat") {
			svcStrat += ".bat"
		}
		snap.ServiceStrategy = svcStrat
	}
	if snap.Strategy == "" && snap.ServiceStrategy != "" {
		snap.Strategy = snap.ServiceStrategy
	}
	...
	m.log.Info("zapret", fmt.Sprintf("CaptureState: стратегия=%s, служба=%v (svc=%s), списков=%d, game_filter=%q, версия=%s", snap.Strategy, snap.WasService, snap.ServiceStrategy, len(snap.UserLists), snap.GameFilter, snap.Version))
	return snap
}

// RestoreState восстанавливает состояние из слепка: пользовательские списки,
// game_filter, стратегию и (если служба была установлена) переустанавливает
// службу с той же стратегией, что была до обновления.
func (m *Manager) RestoreState(snap *BackupSnapshot) error {
	...
	// Prefer the strategy the service was actually installed with.
	strat := snap.ServiceStrategy
	if strat == "" {
		strat = snap.Strategy
	}

	if strat != "" {
		m.cfg.SetCurrentStrategy(strat)
	}

	// Reinstall the service only if it was installed before the update.
	if snap.WasService && strat != "" {
		strategyPath := m.cfg.StrategyPath(strat)
		winws := filepath.Join(m.cfg.BinDir(), "winws.exe")
		if _, err := os.Stat(strategyPath); err == nil {
			if _, err := os.Stat(winws); err == nil {
				m.log.Info("zapret", "RestoreState: переустановка службы "+strat)
				if err := m.InstallService(strat); err != nil {
			...
			m.log.Warn("zapret", "RestoreState: стратегия не найдена: "+strat)
		}
	} else if !snap.WasService {
		m.log.Info("zapret", "RestoreState: служба не была установлена — запуск пропущен")
	}
