...
type XboxDnsConfig struct {
	Enabled      bool   `json:"enabled"`       // Xbox DNS включён
	XboxEnabled  bool   `json:"xbox_enabled"`  // маршрут через xbox-dns.ru
	PrimaryDNS   string `json:"primary_dns"`   // xbox upstream (xbox-dns.ru)
...
type Config struct {
	...
	ServiceCrashCount int   `json:"service_crash_count"`
	ServiceLastCrash  int64 `json:"service_last_crash"`

	AutoSwitchEnabled      bool `json:"auto_switch_enabled"`
	AutoSwitchThresholdPct int  `json:"auto_switch_threshold_pct"`
	AutoSwitchIntervalMin  int  `json:"auto_switch_interval_min"`

	LastAutoSelectTime string `json:"last_auto_select_time"`
...
// SetXboxDnsEnabled переключает флаг работы резолвера без изменения прочих полей.
func (c *Config) SetXboxDnsEnabled(enabled bool) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.XboxDns.Enabled = enabled
	return c.save()
}
...
func (c *Config) ShouldNotify(event string) bool {
	...
	switch event {
	case "zpui_update", "zapret_update":
		// Уведомления об обновлениях управляются единым тумблером «проверка обновлений».
...
func (c *Config) GetServiceCrashCount() int {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.ServiceCrashCount
}

func (c *Config) SetServiceCrashCount(count int) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.ServiceCrashCount = count
	return c.save()
}

func (c *Config) GetServiceLastCrash() int64 {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.ServiceLastCrash
}

func (c *Config) SetServiceLastCrash(t int64) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.ServiceLastCrash = t
	return c.save()
}

func (c *Config) GetAutoSwitchEnabled() bool {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.AutoSwitchEnabled
}

func (c *Config) SetAutoSwitchEnabled(enabled bool) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Config) GetAutoSwitchThresholdPct() int {
	c.mu.RLock()
	defer c.mu.RUnlock()
	pct := c.AutoSwitchThresholdPct
	if pct < 10 || pct > 100 {
		return 50
	}
	return pct
...
func (c *Config) SetAutoSwitchThresholdPct(pct int) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	if pct < 10 {
		pct = 10
	...
	c.AutoSwitchThresholdPct = pct
	return c.save()
}

func (c *Config) GetAutoSwitchIntervalMin() int {
	c.mu.RLock()
	defer c.mu.RUnlock()
	min := c.AutoSwitchIntervalMin
	if min < 1 {
		return 10
	}
	if min > 60 {
...
func (c *Config) SetAutoSwitchIntervalMin(min int) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	if min < 1 {
		min = 1
	...
	c.AutoSwitchIntervalMin = min
	return c.save()
}

func (c *Config) GetLastAutoSelectTime() string {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.LastAutoSelectTime
}

func (c *Config) SetLastAutoSelectTime(t string) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.LastAutoSelectTime = t
	return c.save()
}
