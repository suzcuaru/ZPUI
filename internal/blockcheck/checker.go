...
// NewChecker создаёт Checker с указанными слоями проверки и таймаутом.
// API сохранён совместимым с предыдущей версией: app-код, использующий
// NewChecker(bc.CheckTCP, bc.CheckTLS, bc.CheckHTTP, bc.TimeoutSec),
...
// Check проверяет URL и возвращает результат по слоям (TCP/TLS/HTTP) + вердикт.
// Вердикты:
//
//      OK           — сайт открывается, отдаёт контент ИЛИ сервер ответил
//                     4xx от CDN (TLS прошла, сервер жив — это не блокировка).
//                     Reason уточняет, что именно произошло.
//      HTTP_STUB    — отдаётся заглушка РКН (HTTP 451 или маркеры РКН в теле).
//      TLS_BLOCK    — DPI режет TLS (connection reset, forcibly closed, EOF).
//      TCP_BLOCK    — TCP соединение сброшено.
//      DNS_BLOCK    — DNS не резолвится.
//      TIMEOUT      — таймаут.
//      DOWN         — сервер ответил 5xx (сам сломан).
...
func (c *Checker) checkHTTPGet(fullURL string, result *CheckResult) {
        ...
        // Сохраняем Server-заголовок в Notes — classify будет использовать его,
        // чтобы отличить CDN 4xx (норма) от блокировки.
        server := resp.Header.Get("Server")
        if server != "" {
                result.HTTP.Header = server
        }
}

// classify разбирает результаты слоёв и выставляет итоговый вердикт.
func (c *Checker) classify(result *CheckResult) {
        ...
        if c.checkTLS && !result.TLS.Ok {
                ...
                if isTimeout(result.TLS.Error) {
                        ...
                        result.Confidence = ConfHigh
                        if result.TLS.Error == "connection_reset" {
                                result.Notes = append(result.Notes, "TLS: соединение принудительно сброшено — DPI режет handshake")
                        } else {
        ...
        if c.checkHTTP {
                // 1. Заглушка РКН — HTTP 451 или маркеры РКН в теле.
                ...
                // 3. HTTP 4xx (кроме 451, уже обработанного выше как StubPage).
                // Если сервер ответил 4xx — TLS прошла, соединение установлено,
                // сервер жив. Это НЕ блокировка РКН (РКН режет TLS или DNS, не 4xx).
                // Типичный кейс: корень CDN (etahub.com, r2.1hanime.com) отдаёт
                // 403/404 — это норма.
                if result.HTTP.Status >= 400 && result.HTTP.Status < 500 {
                        server := strings.ToLower(result.HTTP.Header)
                        if containsAny(server, cdnServerMarkers) {
                                result.Verdict = VerdictOK
                                result.Confidence = ConfHigh
                                result.Notes = append(result.Notes,
                                        fmt.Sprintf("HTTP %d от CDN (%s) — сервер работает, URL не отдаёт контент (не блокировка)",
                                                result.HTTP.Status, server))
                                result.HTTP.Ok = true // помечаем как OK для downstream-логики
                                return
                        }
                        // 4xx не от CDN — всё равно не блокировка РКН, сервер жив.
                        result.Verdict = VerdictOK
                        result.Confidence = ConfMedium
                        result.Notes = append(result.Notes,
                                fmt.Sprintf("HTTP %d — сервер отвечает, URL не отдаёт контент (не блокировка)",
                                        result.HTTP.Status))
                        result.HTTP.Ok = true
                        return
                }

                // 4. HTTP 5xx — сервер сам сломан, не наша проблема.
                if result.HTTP.Status >= 500 {
                        result.Verdict = VerdictDown
                        result.Confidence = ConfHigh
                        result.Notes = append(result.Notes,
                                fmt.Sprintf("HTTP %d — серверная ошибка, сайт сломан", result.HTTP.Status))
                        return
                }

                // 5. Ошибка HTTP-запроса (без ответа).
                ...
                if result.HTTP.Error == "connection_reset" {
                        result.Verdict = VerdictTLSBlock
                        result.Confidence = ConfHigh
                        result.Notes = append(result.Notes, "HTTP: соединение принудительно сброшено — DPI режет")
                        return
                }
        }
...
// classifyErr разбирает ошибку в короткий строковый код.
// ВАЖНО: распознаёт Windows-специфичную ошибку DPI
// "wsarecv: An existing connection was forcibly closed by the remote host"
// как connection_reset — без этого Windows-пользователи без zapret видели
// "ERROR" вместо BLOCKED.
func classifyErr(err error) string {
        ...
        case strings.Contains(s, "timeout") || strings.Contains(s, "deadline"):
                return "timeout"
        case strings.Contains(s, "forcibly closed"):
                // Windows: wsarecv: An existing connection was forcibly closed by the remote host.
                // Это Windows-эквивалент connection reset — DPI режет TLS.
...
// detectStubPage консервативно определяет заглушку РКН/TSPU.
// ВАЖНО: только маленькие тела (<4KB) — настоящие сайты обычно отдают
// главную страницу >4KB. Это снижает риск ложных срабатываний на
// юридических/новостных страницах с упоминанием законов.
