...
│   ├── selfupdate/            # обновлятор ZPUI — Wails mini-app (свой React-UI в frontend/, wails.json, окно 480×440)
│   │   ├── main.go            # wails Run, режимы: ручной / --from-app
│   │   ├── app.go             # биндинги: GetState, Download/Apply ZPUI|Zapret, GetReleaseInfo
│   │   └── frontend/          # React + Vite (собирается wails build)
...
- **4 модуля** (`cmd/{wizard,autoselect,selfupdate,zapretupdate}`) — каждый имеет `var version` в `main.go`. **НЕ** бампятся автоматически — ручное изменение при правках модуля (версии читаются из `versions.txt`).
- **`cmd/selfupdate`** — это **Wails mini-app** (свой `frontend/`, `wails.json`), собирается через `wails build` (см. `build.bat`), а не `go build`. Остальные модули — обычный `go build`.
